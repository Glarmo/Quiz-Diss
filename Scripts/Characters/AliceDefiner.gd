...
func _ready():
	...
	var multiply = preload("res://Images/Terminal/multiply.png")
	var noFish1 = preload("res://Images/Gifts/NoFish1.jpg")
	var noFish2 = preload("res://Images/Gifts/NoFish.jpg")
	title = "Alice the Definer"
	dialogue = [["It's about time you learned the real stuff, the bread and butter, the meat and pudding, the lamb and sauce. I could go on but we're pressed for time. We're gonna talk about defining functions and then you're going to do a puzzle for me. So let's start",1],
	["Take a look at this function definition:",2],[squareDef,3],["It should be quite clear what the function does at this point, the first line is the type signature. It can be read as 'square takes type integer and outputs type integer', you should have gone over type declarations with Ali and Dr Composer",4],
	...
	["Go to the computer under the tree and complete the puzzle, you'll be required to finish the function body of the min function. Come back when you've finished it!",8]]
	...
	["One thing of note: The 'x' refers to the first input and 'y' the second. In the example given by the expression 'x' is replaced with '3' and 'y' with '5'. It's important to recognise this as later we will begin to use functions operating on multiple types and you best get them in the right order!",5],
	["Jolly eggs, you should try the next question by the mushrooms and come back one last time for a gift!",6]]
	
	thirdDialogue = [["Another home run! Working with more than one input parameter isn't so bad. I need you to do one last problem for me and then you'll be finised with the level!",1],
	["This last problem should summerise all you've learned so far: Function calling, function composition and function definitions. Good luck!",2],[0,0],["ah! I completely forgot, your gift are these two pictures, I hope you enjoy them as much as I did!",4],
