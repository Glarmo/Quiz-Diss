...
func _ready():
	...
	["In this case 'x > 2' would return a True or False value depending on x. A condition must return a boolean in order to be a condition! We can read the generator as 'for each x drawn from xs, if x greater than 2 return x*x'. While this may have look complicated, hopefully this has demystified the definition",9],
