...
func _ready():
	...
	["This ol lump of coal isn't gonna answer your silly questions",4],
	...
	["Anyway, enough of that. You need to learn about type variables!",5],
	...
	["You may have noticed that the formal parameter looks slightly different. This is because the input is a tuple pair. The formal parameter can be represented in many different ways but most will reflect the structure of the input type. For instance if the input was a list the formal parameter could be '(x:xs)' denoted using brackets and ':' seperating them. The ':' is used in haskell to concatenate an element to the end of a list, x being a single element concatenated to the list xs",11],
