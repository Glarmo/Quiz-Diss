...
func _ready():
	...
	[append,6],["We can use append to connect two lists together, end to end. Remember that a list is of a single type, meaning we can only append two lists of the same type. The same goes for cons. We can use append to add a single element as long as that single element is in a list as you can see from the examples",7], 
