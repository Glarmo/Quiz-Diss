...
func _ready():
	...
	[condition,4],["In this example we take a single int as input and check if it is even, if the number is even we square it and output the result. We could do this operation on a list of ints, let's see what that function would look like",5],
