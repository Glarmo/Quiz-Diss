...
func _ready():
	...
	[maxDef,4],["This is the type signature of the max function. '::' can be read as 'Has type'. Each of the parameters is seperated by '->' the output is the last in the line. So the max function takes two integer inputs and outputs an int aswell.",5],
...
func action(inv):
	if (search(Player_param.problems_completed, "1")):
		dialogue.append(["Not too bad for a monkey, that's all from me I'm afraid. Go back to talk to Dr Composer will you, he was at the bottom of the last island!",10])
