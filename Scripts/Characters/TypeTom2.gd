...
func _ready():
	...
	["Telling you would bring the shine light of day on my terrible dark secret. A secret so dark that the even now I feel Shiva's distant grip tighten on my soul as we speak",4],
	...
	["You may have noticed that the formal parameter looks slightly different. This is because the input is a tuple pair. The formal parameter can be represented in many different ways but most will reflect the structure of the input type. For instance in input was a list the formal parameter could be '[x:xs]' denoted using square brackets and ':' seperating them. The ':' is used in haskell to concatenate (link) two lists together",11],
